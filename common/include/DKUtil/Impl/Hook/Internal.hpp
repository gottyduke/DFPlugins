...
namespace DKUtil::Hook
{
		...
		const bool a_forward = true) noexcept
	{
		...
		if (a_patch.second > (a_offset.second - a_offset.first)) {
			...
			handle->TramPtr = default_trampoline()->allocate<std::uintptr_t>(0);
		...
		model::enumeration<HookFlag> a_flag = HookFlag::kSkipNOP) noexcept
	{
		...
		// [jmp rel32]
		auto tramPtr = default_trampoline()->allocate<std::uintptr_t>();

		WriteImm(tramPtr, a_funcInfo.Address);
		tramPtr += sizeof(a_funcInfo.Address);
		DEBUG("DKU_H: Detouring...\n"
			"from : {}.{:X}\n"
			"to   : {} @ {}.{:X}", GetProcessName(), a_address + a_offset.first, a_funcInfo.Name, PROJECT_NAME, a_funcInfo.Address);
	...
	public:
		...
		IATHookHandle(
			...
			OldAddress(AsPun<std::uintptr_t>(Address))
		{
			DEBUG("DKU_H: IAT @ {:X}\n"
				"old : {} @ {:X}\n"
